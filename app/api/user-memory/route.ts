...
import { getUserProfile, searchUserMemories, getMemoryStore } from "@/lib/langGraphStore";
  ...
  try {
    if (action === 'search' && query) {
      ...
      return NextResponse.json({
        ...
        profile,
        summary: {
          totalTheologicalTopics: Object.keys(profile.theologicalInterests || {}).length,
          totalQuestions: Object.values(profile.questionPatterns || {}).reduce((a, b) => a + b, 0),
          hasPersonalContext: !!profile.personalContext && Object.keys(profile.personalContext).length > 0,
