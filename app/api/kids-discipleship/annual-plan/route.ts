 ...
 * Query params: memberId (optional - filter to one child)
 */
export async function GET(request: Request) {
  ...
  type AnnualPlanChild = (typeof children)[number];
  type ChildAnnualPlan = AnnualPlanChild["annualPlans"][number];

  // Group plans by child
  const result = children.map((child: AnnualPlanChild) => ({
    memberId: child.id,
    childName: child.displayName,
    birthdate: child.birthdate?.toISOString() ?? null,
    plans: child.annualPlans.map((plan: ChildAnnualPlan) => ({
      id: plan.id,
      year: plan.year,
 ...
 * Body: { memberId, year, characterGoal, competencyGoal, ... }
 */
export async function POST(request: Request) {
  ...
  if (!memberId || !year || !characterGoal || !competencyGoal) {
    return NextResponse.json(
      {
        error:
          "Missing required fields: memberId, year, characterGoal, competencyGoal",
      },
      { status: 400 }
    );
  }
