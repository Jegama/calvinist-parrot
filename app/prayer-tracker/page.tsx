...
export default function PrayerTrackerPage() {
  ...
  const [spaceLoadState, setSpaceLoadState] = useState<{ userId: string | null; ready: boolean }>({
    userId: null,
    ready: false,
  });
  ...
  const [newPersonal, setNewPersonal] = useState<NewPersonalFormState>({
    text: "",
    notes: "",
  ...
  async function computeRotation() {
    ...
    const effectiveMembers: Member[] = fetchedMembers && fetchedMembers.length ? fetchedMembers : members;
  ...
  async function reopenRequest(requestId: string, isHouseholdRequest: boolean) {
    if (!user) return;
    setPersonalSheetLoading(true);
    setPersonalSheetError(null);
  ...
  const isCurrentUserSpaceLoaded = Boolean(user && spaceLoadState.ready && spaceLoadState.userId === user.$id);
      ...
      <div className="max-w-6xl mx-auto mt-8 mb-16 space-y-8 px-4 sm:px-6">
        ...
        <RequestSheet
          ...
          onToggleStatus={async (nextStatus) => {
            if (!personalSheet.id) return;
            if (nextStatus === "ACTIVE") {
              const originalLink = personalSheet.originalLinkedToFamily ?? personalSheet.linkedToFamily;
              const isHouseholdRequest = originalLink === "household";
              await reopenRequest(personalSheet.id, isHouseholdRequest);
            }
          }}
