...
export function ChurchList({ items, page, pageSize, total, loading, onPageChange, onSelect }: ChurchListProps) {
  ...
  return (
    <div ref={containerRef} data-church-list>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {items.length === 0 && !loading ? (
              ...
              items.map((church) => {
                ...
                const displayKey: keyof typeof STATUS_STYLES = church.confessionAdopted ? "confessional" : status;
                      ...
                      <div className="order-1 lg:order-2 space-y-1">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {sortedBadges.slice(0, maxBadges).map((badge) => {
                            const badgeInfo = (
                              badgesJson as Record<string, { description?: string; category?: string }>
                            )[badge];
                          ...
                          {sortedBadges.length > maxBadges ? (
                            <Tooltip>
                              ...
                              <TooltipContent side="bottom">View details to see all badges</TooltipContent>
                            </Tooltip>
                          ) : null}
                          {badges.length === 0 ? (
