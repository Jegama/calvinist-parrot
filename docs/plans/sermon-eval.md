...
- Add the following nonsecret names and explanations to `.env.template`; never commit their values:
  - Vercel/Next.js: `APPWRITE_SERMON_FUNCTION_ID` and `APPWRITE_SERMON_BUCKET_ID`, alongside the existing server-side Appwrite endpoint, project ID, and API key. The server key receives only the Function-execution, file-metadata, and file-token capabilities required by the API routes.
  - Appwrite Function: `SERMON_DATABASE_URL`, `GEMINI_API_KEY`, `SERMON_AUDIO_BUCKET_ID`, `SERMON_GEMINI_MODEL`, `SERMON_SOFT_DEADLINE_SECONDS`, `SERMON_MAX_PARALLEL_SCORING_RUNS`, and `SERMON_MAX_ACTIVE_EVALUATIONS`.
